...
static void Run(std::string_view action, unsigned int mask, Point &pos, int &length, Grid &grid)
{
    ...
    int dist;
    [[maybe_unused]]auto result = std::from_chars(action.data() + 1, action.data() + action.size(), dist);
...
void Main()
{
    ...
    assert((Result{135, 410} == Solve("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")));

    auto [closest, fastest] = Solve(inputWire1, inputWire2);
