...
static std::vector<std::vector<int>> FindGroups(gsl::span<int const> ints, int nbPackages)
{
    ...
    for (size_t i = 1; i < max; ++i)
    {
        ...
        Combinations(ints, i, [&](auto const &values) {
...
int64_t GetBestFit(gsl::span<int const> numbers, int nbPackages)
{
    ...
    for (auto &&g : groups)
    {
        auto list = Join(begin(g), end(g), " ", [](auto v) {
    ...
    std::transform(begin(groups), end(groups), std::back_inserter(groupWithQE), [](auto const &g) {
